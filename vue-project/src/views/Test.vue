		...
		<div class="flex-container">
									...
									<span class="custom-tree-node">
										...
										<span>
											<el-button style="right:10px" v-if="data.id === 1" type="text" circle
												size="mini" @click="addNode(node, data)"><el-icon
													class="el-icon--small">
													<Plus />
												</el-icon></el-button>
											<el-button v-if="data.id !== 1" type="text" circle size="mini"
												@click="hideNode(node, data)"><el-icon class="el-icon--small">
													<View />
												</el-icon></el-button>
											<el-button v-if="data.id !== 1" style="margin:0px;" type="text" circle
												size="mini" @click="removeNode(node, data)"><el-icon
													class="el-icon--small">
													<Delete />
												</el-icon></el-button>
										</span>
			...
			<aside class="right-sidebar">
				...
				<div class="rsidebar-main">
					...
					<div v-if="linePos">直线的两点坐标<br /> (x1: {{ formatNumber(pointx1) }}, y1:{{ formatNumber(pointy1) }});
						<br /> (x2:
						{{ formatNumber(pointx2) }}, y2:{{ formatNumber(pointy2) }}); <br /> 直线的长度 : {{
							formatNumber(length1) }} ;
						<br /> 线的角度: {{ angle1 }}度
					</div>
					<div v-if="curvePos">曲线的两点坐标 <br /> (x1: {{ formatNumber(pointx1) }}, y1:{{ formatNumber(pointy1)
						}});<br />
						(x2: {{ formatNumber(pointx2) }}, y2:{{ formatNumber(pointy2) }});<br /> 控制线长度：{{
							formatNumber(length1) }} ;
						{{ formatNumber(length2) }} ;<br /> 线的角度: {{ angle1 }} 度; {{ angle2 }}度</div>
				</div>
